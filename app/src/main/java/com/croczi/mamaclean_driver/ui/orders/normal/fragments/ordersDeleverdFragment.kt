...
class ordersDeleverdFragment : Fragment(),OrderLesener {

    companion object {
        fun newInstance() = ordersDeleverdFragment()
    }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel?.getNormalOrder(MainActivity.cacheObj.token,25,0,"delivered")
        OrdersScroll?.setOnRefreshListener {
            if(flage) {

                viewModel?.getNormalOrder(MainActivity.cacheObj.token,25,0,"delivered")
            }else{
                viewModel?.getPackageOrder(MainActivity.cacheObj.token,25,0,"delivered")
            }
        }
    ...
    override fun onSussessGetNormalOrder(it1: ordersNormalRes) {
        ordersList?.adapter=ItemsnormalOrderAdapter(context!!,it1,viewModel,"delivered")
        OrdersScroll?.isRefreshing=false
    }
    ...
    override fun onSussessGetPackageOrder(it1: packageOrdersRes) {
        ordersList?.adapter=ItemsPackageOrderAdapter(context!!,it1,viewModel,"delivered")
        OrdersScroll?.isRefreshing=false//To change body of created functions use File | Settings | File Templates.
    }
