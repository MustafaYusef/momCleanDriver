...
class ordersCancelFragment : Fragment(),OrderLesener {

    companion object {
        fun newInstance() = ordersCancelFragment()
    }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel?.getNormalOrder(MainActivity.cacheObj.token,25,0,"cancelled")
        ...
        OrdersScroll?.setOnRefreshListener {
            if(flage) {
                ...
                viewModel?.getPackageOrder(MainActivity.cacheObj.token,25,0,"cancelled")
            }
        }
    ...
    override fun onSussessGetNormalOrder(it1: ordersNormalRes) {
        ordersList?.adapter=ItemsnormalOrderAdapter(context!!,it1,viewModel,"cancelled")
        OrdersScroll?.isRefreshing=false
    }
    ...
    override fun onSussessGetPackageOrder(it1: packageOrdersRes) {
        ordersList?.adapter=ItemsPackageOrderAdapter(context!!,it1,viewModel,"cancelled")
        OrdersScroll?.isRefreshing=false//To change body of created functions use File | Settings | File Templates.
    }
