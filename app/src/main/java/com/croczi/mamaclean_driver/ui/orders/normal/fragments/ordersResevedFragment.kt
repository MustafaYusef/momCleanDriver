...
class ordersResevedFragment : Fragment(),OrderLesener {

    companion object {
        fun newInstance() = ordersResevedFragment()
    }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel?.getNormalOrder(MainActivity.cacheObj.token,25,0,"received")
        ...
        OrdersScroll?.setOnRefreshListener {
            if(flage) {
                ...
                viewModel?.getPackageOrder(MainActivity.cacheObj.token,25,0,"received")
            }
        }
    ...
    override fun onFailerNet(message: String) {
        OrdersScroll?.isRefreshing=false
        context?.toast("لا يوجد اتصال")//To change body of created functions use File | Settings | File Templates.
    }

    override fun onSussessGetNormalOrder(it1: ordersNormalRes) {
        ordersList?.adapter=ItemsnormalOrderAdapter(context!!,it1,viewModel,"received")
        OrdersScroll?.isRefreshing=false
    }
    ...
    override fun onSussessGetPackageOrder(it1: packageOrdersRes) {
        ordersList?.adapter=ItemsPackageOrderAdapter(context!!,it1,viewModel,"received")
